...
def compute_jaccard_index(prediction, ground_truth):
    ...
    for result_img,label_img in zip(prediction,ground_truth):
      ...
      img_true=np.where(img_true>0.5,255,0)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_loss < best_loss:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                # calculate the jaccard matrix using the predicted output and the ground truth  labels
                jaccard = compute_jaccard_index(outputs, labels)      
                print("Jaccard Index: for batch {i}",jaccard)  
                i+=1    
                jaccard_score = np.mean(jaccard)
                print('jaccard_score: {:4f}'.format(jaccard_score))
...
def run(UNet,lr=1e-4,step_size=30,gamma=0.1,num_epochs=60):
    set_seed()
    num_class = 6
    ...
    for inputs, labels in test_loader:
      ...
      with torch.set_grad_enabled(False):
          ...
          # calculate the jaccard matrix using the predicted output and the ground truth  labels
          jaccard = compute_jaccard_index(outputs, labels)      
          print("Jaccard Index: for batch {index}",jaccard)      
      index+=1
      # statistics
      epoch_samples += inputs.size(0)
    jaccard_score = np.mean(jaccard)
